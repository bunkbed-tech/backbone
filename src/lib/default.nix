{ pkgs }:
{
  ...
  in contents_new;
  mkResources = { getName ? (resource: resource.metadata.name) }: mkResource:
    pkgs.lib.attrsets.mapAttrs' (_name: obj:
    let resource = mkResource _name obj;
    in pkgs.lib.attrsets.nameValuePair (getName resource) resource
    );
}
